...
return Datatables::of($uniqueConferences)
    ...
    public function downloadReport(Request $request)
    {
        ...
        $query->leftJoin('comments', 'comments.conference', '=', 'conferences.conference');
        ...
        if ($request->user_id == 'All' && $request->conference == 'All') {
            ...
            $query->select('users.id', 'users.name', 'users.created_at', 'conferences.conference', 'conferences.email_sent_status');
            $query->selectRaw('
        ...
        COUNT(CASE WHEN conferences.user_downloaded_at IS NOT NULL THEN conferences.id END) as download_count,
        COUNT( DISTINCT CASE WHEN conferences.email_sent_status = "pending" THEN conferences.id END) as email_pending_count,
        COUNT( DISTINCT CASE WHEN conferences.email_sent_status = "sent" THEN conferences.id END) as email_sent_count  ,
        COUNT(DISTINCT CASE WHEN comments.client_status_id = "1" THEN comments.id END) as client_status_positive_count,
        COUNT(DISTINCT CASE WHEN comments.client_status_id = "2" THEN comments.id END) as client_status_negative_count,
        COUNT(DISTINCT CASE WHEN comments.id IS NOT NULL THEN comments.id END) as comment_count
    ');
    

            $query->whereNotNull('conferences.user_created_at'); // Only count inserted records

            $query->groupBy('users.id', 'users.created_at', 'conferences.conference', 'conferences.email_sent_status');
    ...
    ');
        ...
        if ($request->user_id == 'All' && $request->conference != 'All') {
            ...
            $inserted_count = Conference::whereBetween('user_created_at', [$startDate, $endDate])
                ->where('conference', $request->conference)
            ...
            $downloaded_count = Conference::whereBetween('user_downloaded_at', [$startDate, $endDate])
                ->where('conference', $request->conference)
                ->count();
